...
import { initializeData, persistData, resetDemoData, normalizeStoredData, isRepairReportEmpty, formatRepairReport, DataRepairReport, createEmptyReport } from '../lib/storage';
import {
...
import { appReducer, AppState, Action, deduplicateById } from './appReducer';
...
interface AppContextType {
  ...
  restoreBackup: (backupId: number) => Promise<DataRepairReport>;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const createIdeaCard = async (
    ...
    try {
      ...
      const ideaCard: IdeaCard = {
        ...
        failureConditions: [],
        confounders: [],
        evidenceForHypothesis: evidence.evidenceForHypothesis,
        evidenceAgainstHypothesis: evidence.evidenceAgainstHypothesis,
        falsificationRisks: evidence.falsificationRisks,
  ...
  const createIdeaFromPaper = async (paperId: string): Promise<IdeaCard> => {
    ...
    try {
      ...
      const ideaCard: IdeaCard = {
        ...
        failureConditions: [],
        confounders: [],
        evidenceForHypothesis: ideaData.evidenceForHypothesis,
        evidenceAgainstHypothesis: ideaData.evidenceAgainstHypothesis,
        falsificationRisks: ideaData.falsificationRisks,
  ...
  const handleRestoreBackup = async (backupId: number): Promise<DataRepairReport> => {
    const backup = await getBackup(backupId);
    if (!backup || !backup.data) return createEmptyReport();

    const { data: normalizedData, report } = normalizeStoredData(backup.data);
