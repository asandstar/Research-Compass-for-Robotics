    ...
    return (
      <Card key={paper.id} className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <h4 className="font-medium text-ink dark:text-dark-ink truncate">{paper.title}</h4>
          <p className="text-sm text-muted mt-0.5">
            {paper.authors.split(',')[0]} et al. · {paper.year} · {paper.venue}
          </p>
          {paper.oneSentenceSummary && (
            <p className="text-sm text-ink dark:text-dark-ink mt-2 line-clamp-2">
              {paper.oneSentenceSummary}
            </p>
          )}
        </div>
  ...
  return (
    <Card key={paper.id} className="p-5">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-ink dark:text-dark-ink text-base">
            {paper.title}
          </h3>
          ...
          {paper.relevanceToMyResearch && (
            <div className="mt-3">
              <p className="text-xs text-muted font-medium">和我研究的关系</p>
              <p className="text-sm text-ink dark:text-dark-ink mt-0.5">{paper.relevanceToMyResearch}</p>
            </div>
          )}
          ...
          {(paper.problem || paper.coreContribution || paper.methodSketch) && (
            <>
              ...
              {expanded && (
                <div className="space-y-4 mt-4 pt-4 border-t border-border-subtle dark:border-dark-border-subtle">
                  {paper.problem && (
                    <div className="bg-blue-50 dark:bg-blue-950/20 rounded-lg p-3">
                      ...
                      <p className="text-sm text-ink dark:text-dark-ink">{paper.problem}</p>
                    </div>
                  )}
                  ...
                  {paper.coreContribution && (
                    <div className="bg-green-50 dark:bg-green-950/20 rounded-lg p-3">
                      ...
                      <p className="text-sm text-ink dark:text-dark-ink">{paper.coreContribution}</p>
                    </div>
                  )}
                  ...
                  {paper.methodSketch && (
                    <div className="bg-purple-50 dark:bg-purple-950/20 rounded-lg p-3">
                      ...
                      <p className="text-sm text-ink dark:text-dark-ink whitespace-pre-line">{paper.methodSketch}</p>
                    </div>
                  )}
                  ...
                  {(Boolean(paper.evidence?.tasks?.length) ||
                    ...
                    <div>
                      <p className="text-xs font-medium text-muted mb-2">证据信息</p>
                      <div className="grid grid-cols-2 gap-3">
                        {paper.evidence?.tasks?.length > 0 && (
                          <div className="bg-bg2/50 dark:bg-dark-bg2/50 rounded-lg p-2">
                            <p className="text-xs text-muted mb-1">测试任务</p>
                            <div className="flex flex-wrap gap-1">
                              {paper.evidence.tasks.map((t, i) => (
                                <span key={i} className="text-xs bg-surface dark:bg-dark-surface px-2 py-0.5 rounded text-ink dark:text-dark-ink">
                                  {t}
                                </span>
                              ))}
                            </div>
                        ...
                        {paper.evidence?.baselines?.length > 0 && (
                          <div className="bg-bg2/50 dark:bg-dark-bg2/50 rounded-lg p-2">
                            <p className="text-xs text-muted mb-1">基线方法</p>
                            <div className="flex flex-wrap gap-1">
                        ...
                        {paper.evidence?.metrics?.length > 0 && (
                          <div className="bg-bg2/50 dark:bg-dark-bg2/50 rounded-lg p-2">
                            <p className="text-xs text-muted mb-1">评估指标</p>
                            <div className="flex flex-wrap gap-1">
                        ...
                        {paper.evidence?.keyResults?.length > 0 && (
                          <div className="bg-bg2/50 dark:bg-dark-bg2/50 rounded-lg p-2">
                            <p className="text-xs text-muted mb-1">关键结果</p>
                            <div className="flex flex-wrap gap-1">
                  ...
                  {paper.assumptions?.length > 0 && (
                    <div>
                      <p className="text-xs font-medium text-muted mb-2">假设前提</p>
                  ...
                  {(paper.limitations?.length || paper.limitationsOrQuestions) && (
                    <div className="bg-amber-50 dark:bg-amber-950/20 rounded-lg p-3">
                      ...
                      {paper.limitations?.length ? (
                        ...
                        <p className="text-sm text-ink dark:text-dark-ink">{paper.limitationsOrQuestions}</p>
                      ) : null}
                    </div>
                  )}
                  ...
                  {paper.questionsToVerify?.length > 0 && (
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs font-medium text-muted">待验证问题</p>
                      </div>
                      <ul className="space-y-1">
          ...
          <div className="flex items-center gap-4 mt-4 pt-3 border-t border-border-subtle dark:border-dark-border-subtle">
            {paper.arxivUrl && (
              <a
                href={paper.arxivUrl}
                target="_blank"
        ...
        <div className="flex flex-col items-end gap-2 flex-shrink-0">
          ...
          <div className="text-xs text-muted/70 flex items-center gap-1 mt-2">
          ...
          <div className="flex flex-col gap-1.5 mt-2">
            {onEdit && (
              <button
                ...
                className="p-1.5 text-muted/70 hover:text-ink hover:bg-bg2 dark:hover:bg-dark-bg2 rounded self-end"
                title="编辑"
              >
